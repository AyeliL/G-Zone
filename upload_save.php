...
if ($filedesc == '') {
    $errmsg_arr[] = ' Application description is missing';
    $errflag = true;
}
...
if ($errflag) {
    ...
    session_write_close();
    header("location: student_vacancy.php");
    exit();
}
...
if ((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
    ...
    if (($ext != "exe") && ($_FILES["uploaded_file"]["type"] != "application/x-msdownload")) {
        ...
        $newname = "admin/applications/" . $rd2 . "_" . $filename;
        ...
        if (!file_exists($newname)) {
            ...
            if ((move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $newname))) {
                ...
                // echo "It's done! The file has been saved as: ".$newname;		   
				 ...
				 $qry2 = "INSERT INTO applicants (company_id,student_id,studentname,vacancy_id,vacancy_name,uploads,header,message,adate) VALUES ('$cid','$sid','$sname','$vid','$vname','$newname','$name','$filedesc',NOW())";
				
                //$result = @mysql_query($qry);
                $result2 = $connector->query($qry2);
                if ($result2) {
                    $errmsg_arr[] = 'Application Successfully Submitted';
                    $errflag = true;
                    if ($errflag) {
                        $_SESSION['ERRMSG_ARR'] = $errmsg_arr;
                        session_write_close();
                        ...
                        <!--<script type="text/javascript">
                          window.location="student_vacancy.php<?php //echo '?id='.$id_class; ?>";                          
                        </script>-->
                        <?php
